...
SentinelIQ ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.
...
SentinelIQ employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
SentinelIQ runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
SentinelIQ uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
