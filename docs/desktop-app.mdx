...
- **Auto-update checker** — polls the cloud API for new versions every 6 hours. Displays a non-intrusive update badge with direct download link and per-version dismiss. Variant-aware — a SentinelIQ Tech desktop app links to the correct SentinelIQ Tech release asset
...
SentinelIQ desktop uses a runtime configuration schema with per-feature toggles and secret-backed credentials.
...
- SentinelIQ API key: cloud fallback is blocked; desktop operates local-only.
...
Update prompts are dismissable per-version — dismissing v2.5.0 won't suppress v2.6.0 notifications. The updater is variant-aware: a SentinelIQ Tech desktop build links to the SentinelIQ Tech release asset, not the full variant.
