---
title: "Build on SentinelIQ: Open APIs, Proto-First Architecture, and the Developer Platform"
description: "Build intelligence applications on SentinelIQ's typed API layer. 22 services, 92 proto files, 60+ edge functions, auto-generated TypeScript clients. AGPL-3.0 open source."
metaTitle: "SentinelIQ Developer API: 22 Services, 92 Proto Files, Open Source Intelligence Platform"
...
SentinelIQ is designed differently. The entire intelligence platform, every data feed, every scoring algorithm, every aggregation pipeline, is built on a **typed API layer** that developers can use, extend, and build upon.

## Proto-First Architecture

SentinelIQ uses **Protocol Buffers (protobuf)** as the single source of truth for all API contracts. The codebase contains:
...
For developers building on SentinelIQ, this means you can trust the API contracts completely. If the proto says a field is `int64`, it's `int64`. If it says `repeated string`, it's an array of strings.

## 22 Service Domains

SentinelIQ's API is organized into domain-specific services:
...
## Building with SentinelIQ's API
...
Feed SentinelIQ data into your own analytics:
...
- Build custom scoring models using SentinelIQ's raw data feeds

### Mobile Apps

Build a mobile app that consumes SentinelIQ's API for a custom mobile intelligence experience. The OpenAPI spec makes it accessible from any language (Swift, Kotlin, Python, Go).
...
SentinelIQ is AGPL-3.0. You can self-host the entire platform:
...
For reference, SentinelIQ is built with:
...
SentinelIQ's open, typed, proto-first architecture is the alternative:
...
The intelligence platform of the future isn't a product. It's an ecosystem. SentinelIQ is building the foundation.
