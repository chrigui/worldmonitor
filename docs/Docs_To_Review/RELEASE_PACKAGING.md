...
- Full variant: `SentinelIQ` / `world-monitor`
- Tech variant: `SentinelIQ Tech` / `tech-monitor`

Distinct names are configured in Tauri:

- `src-tauri/tauri.conf.json` → `SentinelIQ` / `world-monitor`
- `src-tauri/tauri.tech.conf.json` → `SentinelIQ Tech` / `tech-monitor`
